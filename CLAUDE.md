...
- **Components folder:** Use `_components/` (not `components/`) for shared components. The underscore prefix keeps it private to the app and consistent with route-level `_components` folders.
- **Never modify shadcn base components:** Files in `_components/ui/` are generated by shadcn. Override styles via `className` props at the call site, never by editing the base component files.
- **Shared domain types live in `lib/`:** Types used across multiple components or between client and API routes belong in a `lib/` module, not exported from component files. When a domain grows beyond a single file, use a directory module and import from specific submodules (e.g., `@/lib/invoice/tokens`, `@/lib/near-intents/status`). No barrel `index.ts` files.
